            ...
            handles.push(tokio::spawn(async move {
                ...
                match result {
                    Ok(mut conn) => {
                        ...
                        if let Ok(Ok(_)) = tokio::time::timeout(timeout, conn.write_all(request.as_bytes())).await {
                            let mut buf = Vec::new();
                            if let Ok(Ok(_)) = tokio::time::timeout(timeout, conn.read_to_end(&mut buf)).await {
                                let latency = start.elapsed().as_millis() as u64;
                                let response = String::from_utf8_lossy(&buf);
                                let alive = response.starts_with("HTTP/1.1 200") || response.starts_with("HTTP/1.0 200");
                                return HealthResult {
